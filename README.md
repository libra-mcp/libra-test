...
- **baseline-ui** — Opinionated UI baseline (Tailwind, accessibility, layout). From [ui-skills baseline-ui](https://www.ui-skills.com/skills/baseline-ui). Includes 6 static rules (h-dvh, icon button aria-label, no arbitrary z-index, 200ms interaction feedback, no gradient/tracking unless requested) and 8 LLM rules (skipped in CLI; run via GitHub App).
