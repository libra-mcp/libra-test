/**
 * Grid layouts should specify gap to avoid cramped cells.
 */
...
export const name = "Grid should include gap";

const EXTENSIONS = [".tsx", ".jsx", ".vue", ".html", ".svelte"];
const GRID_COLS = /grid-cols-[\w\[\]-]+/;
const GAP = /gap-[\w\[\]-]+/;
...
export default async function check(context: RuleContext): Promise<RuleResult> {
  ...
  for await (const filePath of walkByExtension(context.repoPath, EXTENSIONS)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i];
      if (!GRID_COLS.test(line)) continue;
