/**
 * Viewport meta should not disable user scaling (accessibility).
 */
...
export const name = "Do not disable user scaling in viewport";

const EXTENSIONS = [".html", ".htm", ".tsx", ".jsx", ".vue"];
const USER_SCALABLE_NO = /user-scalable\s*=\s*["']?\s*no\s*["']?/i;
...
export default async function check(context: RuleContext): Promise<RuleResult> {
  ...
  for await (const filePath of walkByExtension(context.repoPath, EXTENSIONS)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (!USER_SCALABLE_NO.test(lines[i])) continue;
